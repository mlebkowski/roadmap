...
abstract class UserV2mom implements ActiveRecordInterface
{
    ...
    /**
     * The value for the obstacles field.
     * @var        string
     */
    protected $obstacles;
    ...
    /**
     * Get the [obstacles] column value.
     *
     * @return   string
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : UserV2momTableMap::translateFieldName('Vision', TableMap::TYPE_PHPNAME, $indexType)];
            $this->vision = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : UserV2momTableMap::translateFieldName('Values', TableMap::TYPE_PHPNAME, $indexType)];
            $this->values = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UserV2momTableMap::translateFieldName('Methods', TableMap::TYPE_PHPNAME, $indexType)];
            $this->methods = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UserV2momTableMap::translateFieldName('Obstacles', TableMap::TYPE_PHPNAME, $indexType)];
            $this->obstacles = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UserV2momTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            ...
            return $startcol + 7; // 7 = UserV2momTableMap::NUM_HYDRATE_COLUMNS.
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'METHODS':
                        ...
                        break;
                    case 'OBSTACLES':
                        $stmt->bindValue($identifier, $this->obstacles, PDO::PARAM_STR);
                        break;
                    case 'CREATED_AT':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[2] => $this->getVision(),
            $keys[3] => $this->getValues(),
            $keys[4] => $this->getMethods(),
            $keys[5] => $this->getObstacles(),
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) $this->setVision($arr[$keys[2]]);
        if (array_key_exists($keys[3], $arr)) $this->setValues($arr[$keys[3]]);
        if (array_key_exists($keys[4], $arr)) $this->setMethods($arr[$keys[4]]);
        if (array_key_exists($keys[5], $arr)) $this->setObstacles($arr[$keys[5]]);
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setValues($this->getValues());
        $copyObj->setMethods($this->getMethods());
        $copyObj->setObstacles($this->getObstacles());
